...
function paint(){
    ...
    // Flattened acrylic footprint, shape only (not a surveyed scale).
    color([0.35,0.61,0.69,0.7]);mgraphics.set_line_width(1.5);
    mgraphics.ellipse(cx-81,cy-30,162,60);mgraphics.stroke();
    text('ACRYLIC',cx-82,cy-39,muted,10);
    ...
    text('LIGHT '+Math.round(data[0])+'°',lx+13,ly-10,gold,11);
    ...
    var speakers=[['F1 / F3',cx-80,62],['F2 / F4',cx+65,62],['W1 / W3',w-90,cy-62],['W2 / W4',w-90,cy+62],['C1',70,100],['C2',w-155,100],['C3',70,h-85],['C4',w-155,h-85]];
    for(i=0;i<speakers.length;i++){var v=speakers[i];color(muted);mgraphics.rectangle(v[1],v[2],7,7);mgraphics.stroke();text(v[0],v[1]+12,v[2]+7,muted,10);}
    text('SUB1',w-73,h-49,muted,10);
    text('12.1 proposal · diagram only',18,h-38,muted,10);
}
