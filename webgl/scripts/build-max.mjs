...
const official=[];for(let i=1;i<=8;i++)for(const param of ['xyz','dist','spread','prer','env'])official.push(`/source/${i}/${param}`);for(let i=1;i<=12;i++)official.push(`/speaker/${i}/xyz`);official.push('/speakers/xyz','/sources/xyz');
...
function audio(){const p=patch(1610,1050);note(p,'h','DIRECT A / MULTICHANNEL + VIRTUALSPEAKERS B / 200 ms crossfade',[24,20,1540,30],22);
 ...
 b(p,'multi','spat5.spat~ @inputs 8 @outputs 12 @rooms 1 @internals 8 @initwith "/panning/type knn"',[830,260,760,24],8,13);w(p,'in0','multi');
 for(let i=0;i<8;i++){w(p,'in'+(i+2),'direct',0,i);w(p,'in'+(i+2),'multi',0,i);}
 b(p,'virtual','spat5.virtualspeakers~ @speakers 12',[830,375,520,24],12,3);w(p,'in1','virtual');for(let i=0;i<12;i++)w(p,'multi','virtual',i,i);
...
label('title','φ / DE-SYMMETRICAL ADAPTATION',[25,23,1300,45],27);label('subtitle','WEBGL ↔ OSC ↔ SPAT 5   /   BINAURAL COMPARISON',[25,80,1280,30],18);
...
label('modeA','A   sources → Spat binaural → headphones',[715,367,625,28],14);label('modeB','B   sources → Spat 12 feeds → virtualspeakers~ → headphones',[715,409,625,28],13);
...
label('limit','8 allocated test voices · 12 full-range speaker feeds · SUB1 metadata only · built-in HRTF · source radius 0.2 m',[25,681,1300,30],12);
...
b(p,'oper','spat5.oper @internals 8 @initwith "/source/number 8, /speaker/number 12, /room/number 1, /listener/editable 0"',[545,y,790,26],1,4);w(p,'controller','oper');w(p,'open','oper');
