import {absolute,clamp,relative,type SceneState,type Vec3} from '../model';
...
export function sceneMessages(s:SceneState):OscMessage[]{
 ...
 add(paths.speakers,...s.speakers.slice(0,12).flatMap(p=>relative(p.position,s.listener)));
 add(paths.sub,...s.speakers[12].position);
...
export class SpatOscAdapter{
 ...
 receive(s:SceneState,m:OscMessage):boolean{
  ...
  const accepted=this.apply(s,m);if(accepted)this.cache.set(m.address,JSON.stringify(m.args.map(v=>typeof v==='number'?+v.toFixed(4):v)));return accepted;
 }
 private apply(s:SceneState,m:OscMessage):boolean{
  ...
  if(p===paths.speakers&&nums.length===36&&finite){s.speakers.slice(0,12).forEach((sp,i)=>sp.position=absolute(nums.slice(i*3,i*3+3) as Vec3,s.listener));return true;}
  const sp=/^\/speaker\/(\d+)\/xyz$/.exec(p);if(sp&&finite&&nums.length===3&&+sp[1]>=1&&+sp[1]<=12){s.speakers[+sp[1]-1].position=absolute(xyz(),s.listener);return true;}
  ...
  if(p===paths.sub&&finite&&nums.length===3){s.speakers[12].position=xyz();return true;}
