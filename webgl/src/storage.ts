import {mix,mix3,validateScene,type SceneState} from './model';
export interface SavedPreset {name:string;scene:SceneState;}
export interface Frame {t:number;scene:SceneState;}
const key='dotarea.webgl.presets.v1';
export function loadPresets():SavedPreset[]{try{return parsePresets(JSON.parse(localStorage.getItem(key)||'[]'));}catch{return [];}}
export function parsePresets(v:unknown):SavedPreset[]{if(!Array.isArray(v)||v.length>40)throw Error('Expected at most 40 presets.');return v.map(x=>{if(typeof x.name!=='string'||!x.name.trim()||x.name.length>80)throw Error('Invalid preset name');return {name:x.name,scene:validateScene(x.scene)};});}
...
export function parseRecording(v:unknown):Frame[]{if(!v||typeof v!=='object'||(v as {version:number}).version!==1)throw Error('Unsupported recording');const f=(v as {frames:Frame[]}).frames;if(!Array.isArray(f)||f.length<2||f.length>3000)throw Error('Expected 2–3000 frames');let previous=-1;return f.map(x=>{if(!Number.isFinite(x.t)||x.t<0||x.t<=previous)throw Error('Recording timestamps must strictly increase');previous=x.t;return {t:x.t,scene:validateScene(x.scene)};});}
export function interpolateFrames(frames:Frame[],t:number):SceneState{
 ...
 s.speakers.forEach((sp,k)=>sp.position=mix3(a.scene.speakers[k].position,b.scene.speakers[k].position,f));return s;
}
